...
local function OpenGarageMenu(data)
    QBCore.Functions.TriggerCallback('qb-garages:server:GetGarageVehicles', function(result)
        ...
        -- Get unit once (same for all vehicles)
        local unit = exports["jg-vehiclemileage"]:getUnit() or "kilometers"
        ...
        for _, v in pairs(result) do
            ...
            local imgPath = ('nui://%s/html/images/%s.png'):format(resourceName, model)

            -- === jg-vehiclemileage v2: Get real mileage ===
            local mileageKm = exports["jg-vehiclemileage"]:getMileageByPlate(v.plate)
            if not mileageKm then
                mileageKm = v.drivingdistance or 0  -- Fallback
            end

            -- Convert to miles if needed
            local distance = mileageKm
            local displayUnit = "km"
            if unit == "miles" then
                distance = math.floor((mileageKm / 1.60934) * 10) / 10  -- 1 decimal
                displayUnit = "mi"
            end
            ...
            formattedVehicles[#formattedVehicles + 1] = {
                ...
                body         = bodyPercent,
                distance     = distance,          -- ← Converted value
                distanceUnit = displayUnit,       -- ← "km" or "mi"
...
RegisterNetEvent('qb-garages:client:setHouseGarage', function(house, hasKey)
    ...
    if Config.Garages[formattedHouseName] then
        ...
        QBCore.Functions.TriggerCallback('qb-garages:server:getHouseGarage', function(garageInfo)
